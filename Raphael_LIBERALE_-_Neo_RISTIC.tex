...
Mais le plus long a été la partie d'analyse des complexité. Notamment car les résultats graphiques obtenus paraissaient incorrects, ou du moins non représentatifs des complexités. Au final, nous avons tous les deux résolu le problème ... (pas encore mdr)
...
\begin{itemize}
    \item Tailles testées : $n \in \{$100, 1100, 2100, 3100, 4100, 5100, 6100, 7100, 8100, 9100$\}$ nœuds 
    \item Nombre de répétitions par point de mesure : $50$
...
Le pire cas est atteint premièrement sur un arbre de type filiforme car sa hauteur $h$ est égale à $n-1$, où  on a $n$ le nombre de nœuds de l'arbre. Deuxièmement, pour avoir une complexité dans le pire cas, il faut avoir un arbre où tous les enfants sont des fils gauches, ou sont des fils droits; Dans le cas d'un arbre à $1000$ nœuds (qui est la deuxième plus petite taille testée sur notre échantillon total), on a une probabilité d'avoir un arbre de cette forme-ci qui vaut $\frac{1}{2^h}$., soit dans notre cas $\frac{1}{2^{999}} \approx 0$.

On remarque sur les graphiques qu'en pratique, on atteint une complexité plus proche de $\theta (4n)$; En effet, on a $n_1 + n_2 + 1 + n_3 + n_4$  nœuds concrets 
...
\begin{resultatcle}
    Complexité temporelle de \texttt{est\_abr\_definition} : $\mathcal{O}(\ldots)$ dans le pire cas.
\end{resultatcle}
...
\begin{resultatcle}
    Complexité temporelle de \texttt{est\_abr\_infixe} : $\mathcal{O}(\ldots)$ dans le pire cas.
\end{resultatcle}
